...
keep if flag == "covid_hosp"
keep patient_id indexdate indexMonth practice_id exposed age gender setid

* Load control patients in 2020
append using "data/cohort_`outcome'_control_2020", keep(patient_id practice_id exposed age gender ///
...
noi di "Getting match number `matchnum's"
	...
	forvalues i = 1/`totaltomatch' {
		...
		* Gender, practice, age within 3 yrs, index month 
		frame put if gender==TMgender & practice_id==TMpractice_id & abs(age-TMage)==0, into(eligiblematches)
